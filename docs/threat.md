...
In order to preserve the integrity of data that MiCT stores, processes, or
transmits for Partners, MiCT implements strong intrusion detection tools
...
* MiCT leverages AWS GuardDuty and Cisco Meraki for network security of its on-premise
  environments.
...
* MiCT IT manager is responsible for configuring the firewall and IDS/IPS
  rules and review the configuration as least quarterly.

#### Intrusion Detection in AWS Cloud Environments

MiCT implemented a real-time threat detection solution by
...
landscape for updates to regulations such as EU Privacy Regulations and GDPR.
